...
，定义方差
```
\sigma^2 &= \int_{-\infty}^{\infty}(x-\mu)^2f(x)\text{d}x \\
        &= \int_{-\infty}^{\infty}x^2f(x)\text{d}x - \mu^2 \\
        &= E(x^2) - (E(x)^2)
```
，依然满足「**平方的期望减去期望的平方**」。
...
```
\int u(x)v'(x)\text{d}x &= u(x)v(x) - \int v(x)\text{d}u(x) \\
                         &= u(x)v(x) - \int v(x)u'(x)\text{d}x
```
，可以简写成
```
...
![正态分布](https://s2.loli.net/2023/04/03/ndmR8BLQsAgHN6c.png)
...
另外，由于
```
\Gamma(1) &= \int_0^{\infty}e^{-x}\text{d}x \\
        &=-e^{-x} \mid_0^{\infty} \\
        &= -(0 - 1) \\
        &= 1
```
